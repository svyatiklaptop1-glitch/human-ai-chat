    ...
    <div class="col-span-2 flex flex-col">
      ...
      <div class="flex p-3 border-t border-gray-700">
        <input id="reply" class="flex-1 px-3 py-2 rounded-xl bg-gray-700 text-white" placeholder="Ответ...">
                <button onclick="sendReply()" class="ml-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl">Отправить</button>
      </div>
    </div>
  </div>
...
function connectOperator() {
  ...
  es.addEventListener('new_user_message', ev => {
    const data = JSON.parse(ev.data);
    // Обновляем список
    addOrUpdateUser(data.userId);
...
async function loadHistory(userId) {
  ...
  thread.innerHTML = '';
  d.messages.forEach(m => addMsg(m, thread));
}
...
function renderList(list) {
  ...
  list.forEach(u => {
    ...
    d.className = 'p-2 bg-gray-700 rounded cursor-pointer hover:bg-gray-600 flex justify-between';
    d.textContent = '👤 ' + u.id;
    const span = document.createElement('span');
    span.className = 'text-xs opacity-70';
    span.textContent = u.count;
    d.appendChild(span);
    d.onclick = () => loadHistory(u.id);
    el.appendChild(d);
  });
}
