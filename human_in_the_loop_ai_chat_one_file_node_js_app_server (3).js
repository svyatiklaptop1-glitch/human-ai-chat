/**
 * Human-in-the-Loop “AI” Chat — with avatars + typing indicator + history
 * -------------------------------------------------------------
 * Пользователь думает, что пишет ИИ, а отвечает оператор.
 * Чат сохраняет историю в памяти и подгружает её при обновлении страницы.
 *
 * Запуск:
 *   npm init -y
...
// вернуть историю для пользователя
app.get('/api/history', (req, res) => {
  const convoId = req.conversationId;
  if (!chats.has(convoId)) return res.json({ ok: true, messages: [] });
  res.json({ ok: true, messages: chats.get(convoId).messages });
});
